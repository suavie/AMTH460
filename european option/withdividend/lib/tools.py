...
def binomial_tree_option_price(S, K, T, r, sigma,q, n, option_type='call'):
    T=T/252
    delta_t = T / n
...
def trinomial_tree_option_price(S, K, T, r, sigma,q, n, option_type='call'):
    T=T/252
    delta_t = T / n
    u = np.exp(sigma * np.sqrt(2 * delta_t))
    p_u = ((np.exp((r-q) * delta_t/2) - np.exp(-sigma * np.sqrt(delta_t/2))) /
           (np.exp(sigma * np.sqrt(delta_t/2)) - np.exp(-sigma * np.sqrt(delta_t/2))))**2
    p_d = ((np.exp(sigma * np.sqrt(delta_t/2)) - np.exp((r-q) * delta_t/2)) /
           (np.exp(sigma * np.sqrt(delta_t/2)) - np.exp(-sigma * np.sqrt(delta_t/2))))**2
    p_m = 1 - p_u - p_d
...
def black_scholes_merton(S, K, T, r, sigma,q, option_type='call'):
    ...
    if option_type == 'call':
        option_price = S * norm.cdf(d1)* np.exp(-q*T) - K * np.exp(-r * T) * norm.cdf(d2)
    elif option_type == 'put':
        option_price = K * np.exp(-r * T) * norm.cdf(-d2) - S * norm.cdf(-d1)* np.exp(-q*T)
    else:
        raise ValueError("Invalid option type. Use 'call' or 'put'.")

    return option_price

def monte_carlo_option_price(S, K, T, r, sigma,q,tree_steps,option_type='call', num_simulations=10000):  
    num_steps = tree_steps*100
    T=T/252
    dt=T/num_steps
    ...
    for i in range(num_simulations):
        ...
        stock_path = S * np.exp(np.cumsum((r - q- 0.5 * sigma ** 2) * dt + sigma * np.sqrt(dt) * rand))
    ...
    return option_price
'''stock_path = S * np.exp((r - q- 0.5 * sigma ** 2) * T + sigma * np.sqrt(T) * rand)'''